...
namespace common
{
	...
	void ConsoleApplication::clear_console ()
	{
		...
		// Fill the entire buffer with spaces, using the current colors and attributes.

		if ( !FillConsoleOutputCharacter ( hStdOut, (TCHAR) 0x20, cellCount, homeCoords, &count ) )		return;
	...
	string ConsoleApplication::print_execution_time ( int margin, long long time )
	{
		...
		s << StopClock::time_to_string ( time );		
